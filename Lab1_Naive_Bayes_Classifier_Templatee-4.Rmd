                  ...
                  ,
                      ...
                      score = function(X_test, y_test)
                      {
                        ...
                        if (require(caret, quietly = TRUE)) {
                          ...
                          # Calculate F1 scores for each class with error handling
                          f1_scores <- c()
                          for (class in levels(actual)) {
                            tryCatch({
                              # Check if byClass exists and has the right structure
                              if (!is.null(cm$byClass) && class %in% rownames(cm$byClass)) {
                              ...
                              # Manual calculation if caret fails
                              tp <- sum(predicted == class & actual == class)
                              fp <- sum(predicted == class & actual != class)
                              fn <- sum(predicted != class & actual == class)
                        ...
                        return(list(
                          ...
                          f1_scores = f1_scores,
                          macro_f1 = macro_f1,
                          detailed_metrics = if (require(caret, quietly = TRUE) && !is.null(cm$byClass)) cm$byClass else NULL
                        ))
                      }
                  ))
